...
public class Createligninstruct {
	...
	public static void main(String s[]) {
		...
		Constants.dp_units = 7;
		cls.getLiginList(Constants.dp_units, true);
		Date dt2 = new Date();
	...
	public List<Lignin> getLiginList(int dp, boolean branching) {
		...
		for (int i = 0; i < Constants.bondper.length; i++)
			totalbond += i;
		for (int i : Constants.branchingbndsper)
			totalbond += i;
		...
		if (dp == 2) {
			...
			gp.permute(bondArr, 0, bondArr.length - 1, bondListStr, noOfPer);
