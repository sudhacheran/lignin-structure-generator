...
public class MonolignolBase {
	...
	public void toXYZfile(IAtomContainer mol,int dp, String nm)
	{
		try {

			boolean dirpresent = getDir("xyz", dp);
			FileWriter output = new FileWriter("output/xyz/"+dp+"/lg_" + nm + ".xyz");
			XYZWriter cmlwriter = new XYZWriter(output);
			cmlwriter.write(mol);
			cmlwriter.close();
			output.close();
	...
	public void toCML(IAtomContainer mol,int dp, String nm ) {

		try {

			boolean dirpresent = getDir("cml", dp);
			FileWriter output = new FileWriter("output/cml/"+dp+"/lg_" + nm + ".cml");
	...
	public void toMOL(IAtomContainer mol, int dp, String nm) {
		try {

			boolean dirpresent = getDir("mol/",dp);

			MDLV3000Writer writer = new MDLV3000Writer(new FileWriter(new File("output/mol/"+dp+"/lg_" + nm + ".mol"), false));
			writer.write(mol);
			writer.close();
	...
	public void toIMage(String nm, int dp, IAtomContainer mol) {
		try {
			StructureDiagramGenerator sdg = new StructureDiagramGenerator();
			sdg.generateCoordinates(mol);
			...
			boolean dirpresent = getDir("struct", dp);
			new DepictionGenerator().depict(OutMol) // .withAtomNumbers().withAtomColors()
					.writeTo("output/struct/"+dp+"/lg_" + nm + ".png");
