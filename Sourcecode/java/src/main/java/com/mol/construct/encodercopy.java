...
public class encodercopy {
	...
	static Logger logger = Logger.getLogger(encodercopy.class);
	...
	String[] getList(String[] str, int[] bondper, int dp, int total) {
		...
		// Loop to create the possible bond names (or) monomer labels with the dp count
		int cnt = 0;
		...
		while (!brkLoop) {
			for (int j = 0; j < str.length; j++) {
			    ...
			    //logger.debug("bondper[j]="+bondper[j]+"dp="+dp+"total="+total+"totalcnt[i]="+totcnt[j]+"Cal="+cal);
		...
		//for (String s: strBonds) logger.debug("getlist ->"+s);
		...
		//logger.debug(Arrays.asList(strBonds));
		return strBonds;
	}
	...
	double calPer(int val, int dp, int total) {
		...
		return ((flrval > 0.1 && flrval < 0.5) ? Math.ceil(flrval) : Math.round(flrval));
	}
	...
	public List<String[]> possibleBonds(List<String[]> bondListStr) {
		...
		for (String[] bnds : bondListStr) {
			...
			// sop("Next>> "+retArrr.size());

		}
		//sop("return>> "+retArrr.size());
		return retArrr;
	}
}
